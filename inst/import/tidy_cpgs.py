...
class Read:
    def __init__(self, read1, read2, umi1_idx=None, umi2_idx=None, min_mapq=30, max_insert=1000, infer_umi_field=False):
        ...
        if self.pairing in ['good', 'single_R1', 'single_R2']:
            ...
            if (self.single):            
                self._process_single(read1, read2)
            else:            
    ...
    def _process_single(self, read1, read2):               
        self.umi2 = '-'
    ...
    def _get_umis(self, id, umi1_idx=None, umi2_idx=None, infer_umi_field=False):
        id = id.split(':')
        if umi1_idx == None:
            if infer_umi_field:
                umi1 = filter(lambda x:re.search(r'^umi=', x), id)
                if len(umi1) == 0:
                    umi1 = '-'    
                else:
            ...
            umi1 = id[umi1_idx].translate(None, 'umi=')

        if umi2_idx == None:
            if infer_umi_field:
                umi2 = filter(lambda x:re.search(r'^umi1=', x), id)
                if (len(umi2) == 0):
                    umi2 = '-'
                else:
            ...
            umi2 = id[umi2_idx].translate(None, 'umi1=')
        return([umi1, umi2])
...
########################################################################
def bam_iter(bam, umi1_idx=None, umi2_idx=None, infer_umi_field=False, single_end=False):    
    if single_end:
        for read in bam:
            yield Read(read, None, umi1_idx, umi2_idx, infer_umi_field=infer_umi_field)
    else:
        for read in bam:
            if (read.is_read1):
                read1 = read
            if (read.is_read2 or read.flag & 772):            
                yield Read(read1, read, umi1_idx, umi2_idx, infer_umi_field=infer_umi_field)     

########################################################################
def bam_reader(bam, chunk_size, stats, umi1_idx=None, umi2_idx=None, infer_umi_field=False, chrom=None, genomic_range=None, single_end=False, show_progress=True):    
    while True:
        ids = []
        chroms = []
        starts = []        
        ...
        for read in tqdm(
            bam_iter(bam, umi1_idx=umi1_idx, umi2_idx=umi2_idx, infer_umi_field=infer_umi_field, single_end=single_end),
            unit = 'reads',
            ...
            if read.pairing in ['good', 'single_R1', 'single_R2'] and \
                        ((chrom == None) or (chrom == read.chrom)):
                if (genomic_range == None) \
