...
.gpatterns.ds_stats <- function(track, dsns, bam = NULL, fastq = NULL, intervals=NULL){
    stats <- list()
    ...
    for (dsn in dsns){
        ...
        if (!is.null(fastq)){
            ...
            cmd <- qq('gzip -d -c @{paste(fastq, collapse=" ")} | sed -n 1~4p | head -n @{dsn}')
            ids <- fread(cmd, sep='\t', header=F)[,1] %>%
                gsub('^@', '', .)
        } else if (!is.null(bam)){
            message('sampling from bam')
            cmd <- qq('samtools view @{bam} | sed -n 1~2p | cut -f1 | head -n @{dsn}')
            ids <- fread(cmd, sep='\t', header=F)[,1]
        } else {
        ...
        if (length(ids) == dsn){
            ...
            stats[['umis']] <- stats[['umis']] %>% bind_rows(tibble(dsn=dsn, umis = tcpgs %>% distinct(read_id) %>% nrow))
